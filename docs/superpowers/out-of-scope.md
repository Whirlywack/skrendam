...
- **Live `booking_url` for round-trips (review C2):** `live_backend.search_flights` passes the raw result `f` (a tuple for round-trips) to `client.build_flight_booking_url(f, …)`. Consistent with `fli`'s own MCP usage, but unverified offline — **confirm during the live dry-run**.
