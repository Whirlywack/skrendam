...
| Degraded day | **Log only.** No requeue/retry: retrying into a gating window adds heat exactly when Google is hot; the tail slice self-heals next rotation. The run's health JSON gains a `plan` block (core count, tail count, planned specs) as the tuning instrument. |
...
| New templates | New audience `vfr` ("Visiting friends & family") + `vfr-watch` template (roundtrip, relative ~7–90 days, `included_destinations` enumerating the **actual IATA codes in the seed list** — STN/LTN/LGW/DUB/OSL etc., never "LON"); `long-haul-opportunist` (LONG_HAUL zone, roundtrip, wide relative window, min_discount 30%, max_stops 2, gate-exempt). |
...
- Seeds: idempotent re-run; founder-disabled route stays disabled; every enabled
  template has ≥1 core route (asserted as a test, not a hope).
