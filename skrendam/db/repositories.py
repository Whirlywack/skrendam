...
def upsert_candidate(
    session: Session, deal_group_key: str, fields: dict, now: datetime
    ...
    if existing is None:
        try:
            with session.begin_nested():  # savepoint: a lost race must not poison the run
                cand = models.Candidate(
                    deal_group_key=deal_group_key,
                    first_seen_at=now,
                    last_seen_at=now,
                    status="new",
                    **fields,
            ...
            # A concurrent scan (daily cron vs curator-triggered) inserted the same
            # deal_group_key between our select and flush; update the winner's row.
            existing = session.scalar(
                select(models.Candidate).where(models.Candidate.deal_group_key == deal_group_key)
            )
            if existing is None:
...
def upsert_score(
    session: Session, candidate_id: int, template_id: int, score
) -> tuple[models.CandidateScore, bool]:
...
def ensure_content_draft(
    session: Session, candidate_id: int, template_id: int, draft: dict
) -> models.ContentDraft:
