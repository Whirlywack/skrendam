...
# Changed paths = committed since branch point ∪ uncommitted working tree.
CHANGED="$( { git diff --name-only "$MB" HEAD; git status --porcelain | awk '{print $NF}'; } | sort -u )"
N_CHANGED="$(printf '%s\n' "$CHANGED" | grep -c . || true)"
...
if has '^(fli-js/|data/)'; then
  ...
  if command -v bun >/dev/null 2>&1; then
    ...
    if has '^data/'; then
      step "enum codegen drift" bash -c 'cd fli-js && bun run generate:enums && git diff --exit-code src/models'
    fi
  else
