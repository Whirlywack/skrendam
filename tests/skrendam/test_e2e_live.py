...
def test_live_full_scan_end_to_end():
    ...
    so that at most a handful of real API calls are made.  Live prices vary so we
    ...
    try:
        ...
        session.add(models.Zone(
            zone="CITY_BREAKS", haul_type="short",
            threshold_price_eur=45, min_abs_savings_eur=20, min_discount_pct=25,
        ))
        session.add(models.Route(origin="VNO", destination="STN", zone="CITY_BREAKS", enabled=True))
        aud = models.AudienceSegment(slug="budget_e2e", name="Budget (e2e)",
                                     default_itinerary_tolerance="relaxed")
        mom = models.TravelMoment(slug="lm_e2e", name="Last-minute (e2e)",
                                  moment_type="relative",
                                  default_content_angle="Leave this weekend")
        session.add_all([aud, mom])
        session.flush()
        session.add(models.DealTemplate(
            slug="e2e-lm-oneway", name="E2E last-minute oneway",
            enabled=True, audience_segment_id=aud.id, travel_moment_id=mom.id,
            trip_type="oneway", date_window_type="relative",
            rel_offset_start_days=3, rel_offset_end_days=60,
            included_zones=["CITY_BREAKS"], max_stops=1,
        ...
        summary = run_scan(session, today=date.today(), adapter=adapter,
                           scanner_version="e2e")
