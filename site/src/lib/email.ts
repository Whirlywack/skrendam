 ...
 *  click — a third party typing their email must not flip the flag silently
 *  (code review 2026-08-28). No-op when email is disabled. */
export async function sendEarlyConfirmEmail(email: string, token: string): Promise<void> {
  if (!emailEnabled()) return;
  const resend = new Resend(process.env.RESEND_API_KEY!);
  const url = `${siteUrl()}/confirm?token=${encodeURIComponent(token)}&early=1`;
  await resend.emails.send({
    from: FROM,
    to: email,
    subject: 'Patvirtink skubias žinutes — Yip',
    text: `Norėjai gauti ir skubias žinutes — rečiausius radinius iškart, kai tik juos randam. Patvirtink paspausdamas:\n\n${url}\n\nJei to neprašei — tiesiog ignoruok šį laišką, niekas nepasikeis.`,
    html: `<!DOCTYPE html>
<html lang="lt">
...
<body style="margin:0;padding:0;background:#FFFDF7;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',sans-serif;">
  <div style="max-width:520px;margin:0 auto;padding:48px 24px;">
    ...
    <p style="margin:0 0 24px;font-size:16px;line-height:1.6;color:#1C1813;">
      Norėjai gauti ir skubias žinutes — rečiausius radinius iškart, kai tik juos randam, dar prieš savaitinį laišką. Patvirtink vienu paspaudimu.
    </p>
    <a href="${url}"
