...
import { upsertMoment } from '@/app/config-actions';
...
interface MomentRow {
  ...
  momentType: string;
  defaultContentAngle: string | null;
}

interface MomentFormProps {
  /** Existing row to edit, or null to create a new moment. */
  moment: MomentRow | null;
}

export function MomentForm({ moment }: MomentFormProps) {
  const isEdit = moment !== null;
  ...
  return (
    ...
    >
      {/* Hidden id for edits */}
      {isEdit && <input type="hidden" name="id" value={moment.id} />}

      {/* Row 1: slug + name + moment_type */}
      <div style={{ display: 'flex', gap: 12, flexWrap: 'wrap', alignItems: 'flex-end' }}>
        ...
        <label style={labelStyle}>
          <span style={labelTextStyle}>Moment type</span>
          <select
            name="moment_type"
            defaultValue={moment?.momentType ?? 'relative'}
      ...
      <label style={{ ...labelStyle, width: '100%' }}>
        ...
        <textarea
          ...
          placeholder="Moment description (optional)"
          rows={2}
          style={{
            ...inputStyle,
            width: '100%',
      ...
      {/* Row 3: default_content_angle */}
      <label style={{ ...labelStyle, width: '100%' }}>
        <span style={labelTextStyle}>Default content angle</span>
        <textarea
          name="default_content_angle"
          defaultValue={moment?.defaultContentAngle ?? ''}
          placeholder="Default copywriting angle for this travel moment (optional)"
          rows={3}
          style={{
            ...inputStyle,
            width: '100%',
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
        ...
        >
          {isPending ? 'Saving…' : isEdit ? 'Save changes' : 'Create moment'}
        </button>
